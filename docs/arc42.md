...
| バックエンド実装はGo（chi + sqlx） | 性能改善プロジェクトの一環として正式採用した本番実装（9節ADR-2参照） |
...
**過去に存在した他言語実装について:** リポジトリには現在もRuby/PHP/Python/Node.jsの実装が残存しているが、本番で稼働しているのはGo実装のみである（4節・ADR-2参照）。未使用実装の保守コストは技術的負債として11節で扱う。
...
### ADR-1: セッションストアにMemcachedを採用する

**日付:** 不明（既存実装からの継承）
**ステータス:** 採用

**コンテキスト:**
セッションをDBに保存するとDB負荷が増える一方、Cookieに全情報を持たせるとセキュリティ・サイズの制約が生じる。

**決定:**
`gorilla/sessions` + `gorilla-sessions-memcache`によりセッションデータをMemcachedへ保存する。

**検討した代替案:**
- DBセッション: 実装は単純だがDB負荷が増加する
- Cookieのみ（署名付き）: サーバー側状態を持たない利点はあるが、BANのような「即座にサーバー側で無効化したい」状態管理と相性が悪い

**結果:**
- ✅ DBへの追加負荷なくセッションを高速に読み書きできる
- ⚠️ セッション暗号鍵の管理方法に課題が残っている（8節・11節R2参照）

### ADR-2: バックエンド実装言語としてGoを正式採用する

**日付:** 2026-09-13
**ステータス:** 採用

**コンテキスト:**
リポジトリには過去複数言語の実装が並行して存在していたが、保守コストの増大と、誤った実装のデプロイリスクを避けるため、本番実装を1つに統一する必要があった。

**決定:**
Go実装（chi + sqlx + gomemcache）を正式な本番実装として採用し、デプロイ対象を`webapp/golang`に一本化する。

**検討した代替案:**
- Ruby実装を継続利用: 既存の稼働実績はあるが、性能改善プロジェクトの目標（レイテンシ削減）に対してGoの方が有利と判断
- 全言語実装を並行維持: 学習コスト・保守コストが高く、本番運用の観点では非合理

**結果:**
- ✅ Nginx → Go app → MySQL/Memcached という単純な経路に統一でき、性能チューニングの見通しが立てやすくなった
- ✅ `go build`/`go vet`がクリーンに通ることを確認済みで、CI・ローカル開発の両方で扱いやすい
- ⚠️ 他言語実装（Ruby/PHP/Python/Node.js）がリポジトリに残存しており、順次廃止する作業が必要（11節R6参照）
